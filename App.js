...
export default function App() {
  ...
  const scan = async () => {
    ...
    setImg(res.assets[0].uri); setProc(true); setVend(''); setAmt(''); setRem('');
    try {
      ...
      setVend(txt.split('\n')[0]?.substring(0,25) || 'Bill Entry'); setAmt(bA); setRem('Captured via OCR');
