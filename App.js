...
export default function App() {
  ...
  const scan = async () => {
    ...
    try {
      ...
      setV(txt.split('\n')[0]?.substring(0,25) || 'Bill'); setAmt(bA);
    } catch(e) { console.log(e); }
    setIsP(false); setMV(true);
  };

  if(!ready) return <View style={styles.lC}><ActivityIndicator size="large" color="#FFD700"/><Text style={styles.lT}>SBA Accounts Starting...</Text></View>;
