...
export default function App() {
  ...
  useEffect(() => { setTimeout(() => setR(true), 1000); }, []);
  ...
  const scan = async () => {
    ...
    try {
      ...
      setV(txt.split('\n')[0]?.substring(0,25) || ''); setAmt(bA); setRem('Captured');
    } catch(e) { console.log(e); }
    setIsP(false); setMV(true);
  };

  if(!r) return <View style={s.lC}><ActivityIndicator size="large" color="#FFD700"/><Text style={s.lT}>SBA Accounts Loading...</Text></View>;
